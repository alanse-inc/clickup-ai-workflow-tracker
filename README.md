...
本リポジトリの `.github/workflows/agent.yaml` を、ターゲットリポジトリ（Claude Code が実装を行うリポジトリ）にコピーして使用します。

ClickUp リストの作成、GitHub Secrets の設定、動作確認の手順は **[agent.yaml セットアップガイド](./docs/agent-setup-guide.md)** を参照してください。

> **推奨**: `GITHUB_APP_ID` / `GITHUB_APP_PRIVATE_KEY`（オーケストレーターと同じ GitHub App の値）を設定すると、Claude Code が作成した PR で CI が自動トリガーされます。Organization Secret として設定する場合は組織名を含む名前に変更してください。詳細はセットアップガイドを参照してください。

### 2. オーケストレーターのプロジェクト設定（projects.yaml）

`projects.yaml` で ClickUp リストと GitHub リポジトリの対応を設定します。
