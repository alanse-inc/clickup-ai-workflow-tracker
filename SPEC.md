...
Symphony では `WORKFLOW.md` にプロンプトテンプレートと設定を一体で管理するが、本システムではその役割を以下に分離する:
...
- uses: anthropics/claude-code-action@v1
  with:
    prompt: |
      ClickUp タスク ID: ${{ steps.fetch_task.outputs.task_id }}
      タスク名: ${{ steps.fetch_task.outputs.task_name }}
      タスク内容:
      ${{ steps.fetch_task.outputs.task_description }}

      # SPEC フェーズの場合:
      上記タスク内容をもとに、実装仕様書を Markdown で作成してください。
      ...
```
