...
オーケストレータから渡される入力は `task_id` と `phase` のみ。タスクの詳細情報はワークフロー内で ClickUp API から直接取得する。
...
- **処理**:
  ...
  4. ClickUp API でステータスを `Spec Review` に変更。
- **Output**: タスクの Description に仕様書が記載され、ステータスが `Spec Review` に遷移。
...
- **処理**:
  ...
  3. ワークフロー内で ClickUp API を叩き、ステータスを `PR Review` に変更。
  4. ClickUp API で GitHub PR URL カスタムフィールドに PR リンクを設定。
- **Output**: PR が作成され、ステータスが `PR Review` に遷移、PR リンクがタスクに記録。
