...
作業対象リポジトリの `.github/workflows/agent.yml` に定義する。
...
ステータス名の定義はオーケストレータ（Go Server）に一元化する。`agent.yml` はステータス名をハードコードせず、`inputs.status_on_success` / `inputs.status_on_error` をそのまま ClickUp API に渡す。
...
`agent.yml` 内の最初のステップで ClickUp API を叩き、タスク情報を取得する。取得した情報は後続ステップの `prompt` に埋め込む。
...
- **プロンプト**: `agent.yml` 内で Claude Code Action の `prompt` パラメータにインラインで指定。
- **プロジェクト固有ルール**: 作業対象リポジトリの `CLAUDE.md` から Claude Code が自動的に読み込む。

`agent.yml` 内で `inputs.phase` に応じてプロンプトを切り替える:
...
プロンプトの変更は `agent.yml` の編集だけで完結する。別途テンプレートファイルを管理する必要はない。
