...
本システムは環境変数によって ClickUp リストと GitHub リポジトリを 1:1 で紐づける。

- `CLICKUP_LIST_ID` が 1 つの ClickUp リスト（= 1 つのプロジェクトボード）を指定する。
- `GITHUB_OWNER` + `GITHUB_REPO` が 1 つの GitHub リポジトリを指定する。
...
Symphony では `WORKFLOW.md` にプロンプトテンプレートと設定を一体で管理するが、本システムでは以下の理由で分離する:

- **設定**: 環境変数で管理（Section 6 参照）。
- **プロンプトテンプレート**: `.github/prompts/` 配下にフェーズごとのファイルとして管理。
- **プロジェクト固有ルール**: リポジトリルートの `CLAUDE.md` から Claude Code が自動的に読み込む。

```
.github/prompts/
├── spec.md   # SPEC フェーズ用プロンプトテンプレート
└── code.md   # CODE フェーズ用プロンプトテンプレート
```

`agent.yml` ワークフロー内で `inputs.phase` に応じてテンプレートを読み込み、ClickUp タスク情報を埋め込んで Claude Code に渡す。

テンプレート内で利用可能な変数:

- `{{task_id}}`: ClickUp タスク ID
- `{{task_name}}`: タスク名
- `{{task_description}}`: タスクの説明文（SPEC フェーズでは元のアイデア、CODE フェーズでは仕様書）

これにより、プロンプトの変更はコードの変更なしにテンプレートファイルの編集だけで完結する。

### 9.6 Error Handling in GitHub Actions
...
├── .github/
│   ├── prompts/
│   │   ├── spec.md            # SPECフェーズ用プロンプトテンプレート
│   │   └── code.md            # CODEフェーズ用プロンプトテンプレート
│   └── workflows/
