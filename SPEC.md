...
4. **PR Checker** (PR Merge Detector)
   - GitHub API を介して PR のマージ状態を確認する。
   - ブランチ名規約 `feature/clickup-{taskID}` から対応する PR を検索する。
   - リコンシリエーション時に `PR Review` ステータスのタスクに対してマージ検知を行い、マージ済みなら `CLOSED` に自動遷移する。
...
| 7 | PR Review | 人間 | PR作成完了。人間がコードレビューを行いマージする。マージ後、オーケストレータが自動で CLOSED に遷移する。 |
| 8 | CLOSED | 自動/人間 | すべての作業が完了。PR マージ時にオーケストレータが自動遷移、または人間が手動で移動。 |

### 5.1.1 Shortcut Flow (Skip SPEC)

軽微なバグ修正やリファクタリングなど仕様策定が不要なタスクでは、SPEC フェーズをスキップして直接実装に進むことができる:

```
Idea Draft -> Ready for Code -> Implementing -> PR Review -> CLOSED
```

この場合、タスクの Description に正式な仕様書は存在しない。CODE フェーズのエージェントはタスク名と簡易説明をもとに、リポジトリの既存コードを踏まえて適切に実装する。
...
- **作業対象リポジトリ**: プロジェクト設定で指定される任意のリポジトリ。Claude Code が実際にコードを読み書きする対象。作業対象リポジトリに `agent.yaml` を配置する必要がある。

#### 複数リポジトリ対応

1 つのオーケストレータインスタンスで複数の ClickUp リスト - GitHub リポジトリペアを管理できる。プロジェクト定義は YAML ファイル (`projects.yaml`) で管理し、API トークンや認証情報は環境変数で管理する。

**YAML 設定ファイル** (`PROJECTS_FILE` 環境変数でパス指定、default: `projects.yaml`):
...
3. GitHub API経由で `workflow_dispatch` をトリガー:
   ...
   - `inputs.status_on_success` = 成功時の遷移先ステータス（`spec review` or `pr review`）
   - `inputs.status_on_error` = エラー時の戻し先ステータス（`ready for spec` or `ready for code`）
...
- ステータスが `pr review` かつ PRChecker が有効な場合: GitHub API で対応する PR のマージ状態を確認し、マージ済みであれば ClickUp タスクを `closed` に自動更新してクレーム解除。未マージであれば処理中として維持。
...
作業対象リポジトリの `.github/workflows/agent.yaml` に定義する。
オーケストレータ（本リポジトリ）ではなく、Claude Code が実際にコードを読み書きするリポジトリに配置する。

### 9.2 Workflow Dispatch Inputs

オーケストレータから `task_id`、`phase`、ステータス遷移先を渡す。タスクの詳細情報はワークフロー内で ClickUp API から直接取得する。
  ...
  workflow_dispatch:
    inputs:
      ...
      phase:
        ...
        type: string
      status_on_success:
        description: 'ClickUp status to set on success (e.g. spec review, pr review)'
        required: true
        type: string
      status_on_error:
...
### 9.3 Required Secrets on Target Repository

作業対象リポジトリの GitHub Secrets に以下を設定する:

| Secret | 用途 |
|--------|------|
| `CLICKUP_API_TOKEN` | ClickUp API でタスク取得・ステータス更新・フィールド更新に使用 |
| `ANTHROPIC_API_KEY` | Claude Code Action の実行に使用 |

`GITHUB_TOKEN` は GitHub Actions が自動付与するため設定不要。

### 9.4 ClickUp Task Fetch Step

`agent.yaml` 内の最初のステップで ClickUp API を叩き、タスク情報を取得する。取得した情報は後続ステップの `prompt` に埋め込む。
...
- name: Fetch ClickUp task
  ...
  run: |
    RESPONSE=$(curl -s "https://api.clickup.com/api/v2/task/${TASK_ID}" \
      -H "Authorization: ${CLICKUP_API_TOKEN}")
    echo "task_name=$(echo "$RESPONSE" | jq -r '.name')" >> "$GITHUB_OUTPUT"
    echo "task_description<<EOF" >> "$GITHUB_OUTPUT"
    echo "$RESPONSE" | jq -r '.description // ""' >> "$GITHUB_OUTPUT"
...
- **Input**: `task_id`, `phase=CODE`, `status_on_success=pr review`, `status_on_error=ready for code`
- **処理**:
  1. ClickUp API からタスク内容（仕様書含む）を取得（Section 9.4）。
  2. Claude Code Action に仕様書を prompt として渡し、以下を実行させる:
     - `feature/clickup-{task_id}` ブランチ作成。
     - コード変更・テスト実行。
     - コミット＆Push。
     - `gh pr create` で PR 作成。
  3. ワークフロー内で ClickUp API を叩き、ステータスを `inputs.status_on_success` に変更。
  4. ClickUp API で GitHub PR URL カスタムフィールドに PR リンクを設定。
- **Output**: PR が作成され、ステータスが遷移、PR リンクがタスクに記録。

### 9.7 Prompt Design

Symphony では `WORKFLOW.md` にプロンプトと設定を一体で管理するが、本システムではその役割を分離する:

- **設定**: 環境変数で管理（Section 6 参照）。
- **プロンプト**: `agent.yaml` 内で Claude Code Action の `prompt` パラメータにインラインで指定。
- **プロジェクト固有ルール**: 作業対象リポジトリの `CLAUDE.md` から Claude Code が自動的に読み込む。

`agent.yaml` 内で `inputs.phase` に応じてプロンプトを切り替える:

```yaml
- name: Run Claude Code (SPEC)
  if: inputs.phase == 'SPEC'
  uses: anthropics/claude-code-action@v1
  with:
    prompt: |
      以下の ClickUp タスクの内容をもとに、実装仕様書を Markdown で作成してください。

      タスク名: ${{ steps.fetch_task.outputs.task_name }}
      タスク内容:
      ${{ steps.fetch_task.outputs.task_description }}
      ...

- name: Run Claude Code (CODE)
  if: inputs.phase == 'CODE'
  uses: anthropics/claude-code-action@v1
  with:
    prompt: |
      以下の仕様に基づいてコードを実装し、PR を作成してください。
...
プロンプトの変更は `agent.yaml` の編集だけで完結する。別途テンプレートファイルを管理する必要はない。

### 9.8 Error Handling in GitHub Actions

- エージェント実行中にエラーが発生した場合:
  1. ClickUp API で Agent Error カスタムフィールドにエラーメッセージを記録。
  2. ClickUp API でステータスを `inputs.status_on_error` に戻す。
...
│   │   └── pr_checker.go      # GitHub PR マージ状態チェック
│   ├── orchestrator/
...
├── .github/
│   └── workflows/
│       └── agent.yaml          # Claude Code実行ワークフロー
├── go.mod
├── go.sum
├── SPEC.md                    # 本ドキュメント
└── .gitignore
