...
<details>
<summary>CLICKUP_API_TOKEN の取得手順</summary>
...
<details>
<summary>CI_APP_ID / CI_APP_PRIVATE_KEY の設定手順</summary>

1. オーケストレーターの環境変数に設定済みの `GITHUB_APP_ID` と `GITHUB_APP_PRIVATE_KEY` の値を取得
2. 対象リポジトリまたは Organization の Secrets に `CI_APP_ID` / `CI_APP_PRIVATE_KEY` として登録
</details>
