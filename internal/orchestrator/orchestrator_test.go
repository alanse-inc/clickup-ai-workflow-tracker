...
func TestNew_NilLoggerFallback(t *testing.T) {
	...
	o := New(fetcher, dispatcher, Config{PollInterval: time.Second, StatusMapping: defaultSM}, nil, NewAgentState())
...
func TestTick_DispatchesTriggerStatusTasks(t *testing.T) {
	...
	dispatcher := &mockWorkflowDispatcher{}
	o := New(fetcher, dispatcher, Config{PollInterval: time.Second, StatusMapping: sm}, defaultLogger, NewAgentState())
...
func TestTick_GetTasksError(t *testing.T) {
	...
	dispatcher := &mockWorkflowDispatcher{}
	o := New(fetcher, dispatcher, Config{PollInterval: time.Second, StatusMapping: defaultSM}, defaultLogger, NewAgentState())
...
func TestRun_StopsOnContextCancel(t *testing.T) {
	...
	dispatcher := &mockWorkflowDispatcher{}
	o := New(fetcher, dispatcher, Config{PollInterval: 50 * time.Millisecond, StatusMapping: defaultSM}, defaultLogger, NewAgentState())
...
func TestTick_MaxConcurrentTasksLimit(t *testing.T) {
	...
	dispatcher := &mockWorkflowDispatcher{}
	o := New(fetcher, dispatcher, Config{PollInterval: time.Second, StatusMapping: sm, MaxConcurrentTasks: 2}, defaultLogger, NewAgentState())

	// task-running は既に実行中（上限に向けてカウントされる）
	o.state.Claim("task-running")
	o.state.MarkRunning("task-running")

	// ActiveCount=1 の状態で上限=2 → あと1つだけディスパッチできる
	o.tick(context.Background())

	dispatcher.mu.Lock()
	defer dispatcher.mu.Unlock()
	if len(dispatcher.triggerCalls) != 1 {
		t.Fatalf("expected 1 trigger call (limit=2, 1 already running), got %d", len(dispatcher.triggerCalls))
	}
}

func TestTick_MaxConcurrentTasksZeroIsUnlimited(t *testing.T) {
	sm := defaultSM
	fetcher := &mockTaskClient{
		tasks: []clickup.Task{
			{ID: "task-1", Status: sm.ReadyForSpec},
